...
import { validateEmail, encodeText, createFormObj } from "../utils.js";
...
function setTextInputValue(inputField, e) {
  // sets the textVal of usernameInput, emailInput, or passwordInput
  this[inputField].textVal = e.target.value;

  if (this[inputField].hasError) {
    // reset error status per field on text change
...
watch(
  () => route.path,
  (newPath) => {
    isCreateAcctPage.value = newPath === "/create-account";
  },
);
</script>
